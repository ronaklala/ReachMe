...
const UserSearch = () => {
  ...
  const getuser = async () => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        onsearch1(res.data.filter((user) =>user.username.toString().toLowerCase().includes(search.search.toString().toLowerCase())));
        console.log(serach1)
  ...
  return (
    ...
    {loading == true ? ( 
    <>
    <center><SyncLoader color="#ffffff"
                  loading={loading}
                  css={override}
                  size={50}/>
                  <br />
